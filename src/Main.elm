...
init : (Struc, Cmd msg)    
init = ({ roll_c = 200 , roll_p = 200 , pot = 0 , deck = []} , Cmd.none)
...
type Msg = Shuffle | Show (List Int) | Bet | AllIn
                  
update : Msg -> Struc -> (Struc , Cmd Msg)
update b m = case b of
    AllIn ->
        if m.roll_p > m.roll_c
        then ({ m | pot = m.pot + m.roll_c , roll_p = m.roll_p - m.roll_c } , Cmd.none)
        else ({ m | pot = m.pot + m.roll_p , roll_p = 0 } , Cmd.none)    
    Bet ->
        ({m | pot = m.pot + 1 , roll_p = m.roll_p - 1 } , Cmd.none)
    Shuffle ->
        (m , Random.generate Show (Random.list 2048 (Random.int 2 53)))
    Show m ->
        ({ roll_c = 200 , roll_p = 200 , pot = 0 , deck = List.Extra.unique m } , Cmd.none)
